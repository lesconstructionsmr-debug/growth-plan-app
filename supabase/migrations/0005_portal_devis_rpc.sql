-- Portail devis client : accès public contrôlé par portal_token
-- L'accès direct sur public.devis reste bloqué par devis_isolation (RLS).
-- Seules ces fonctions SECURITY DEFINER exposent les données, sans notes_internes.
...
begin
  ...
  set
    ...
    signature_data = case
      when p_action = 'approuve' and p_signature_data is not null then p_signature_data
      else signature_data
    end,
    signataire_nom = case
      when p_action = 'approuve' and p_signataire_nom is not null then p_signataire_nom
      else signataire_nom
    end,
    signe_le = case
      when p_action = 'approuve' and p_signature_data is not null then now()
      else signe_le
    end
  where portal_token = p_token;
